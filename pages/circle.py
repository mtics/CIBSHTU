...
workbook = xlrd.open_workbook('上科大猫咪档案.xlsx')
...
labels = [
    ...
    [9, '状态', lambda x: '不明' if len(x) < 1 else x],
    [10, '绝育情况', lambda x: '已绝育' if x == 1 else '未绝育' if x == 0 else '未知/可能不适宜绝育'],
    [11, '绝育时间', lambda x: str(x)],
    [12, '出生时间', lambda x: x],
    [14, '性格', lambda
        x: '亲人可抱' if x == 6 else '亲人不可抱 可摸' if x == 5 else '薛定谔亲人' if x == 4 else '吃东西时可以一直摸' if x == 3 else '吃东西时可以摸一下' if x == 2 else '怕人 安全距离 1m 以内' if x == 1 else '怕人 安全距离 1m 以外' if x == 0 else '未知 数据缺失'],
...
for line in data_json:
    if line['是否写入图鉴'] != '':
        if not os.path.exists(r'cats/' + line['名字']):
            os.makedirs(r'cats/' + line['名字'])  # 创建每只猫的文件夹
        # 创建js文件
        with open(r'cats/' + line['名字'] + '/' + line['名字'] + '.js', 'w', encoding='utf-8') as f:
            f.write('Page({ \n data: {\n' 'catname:"' + line['名字'] + '",\n catitems:[ \n')
            for j in labels:
                # 打印名字
                if j[1] == '名字':
